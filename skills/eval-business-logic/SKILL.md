...
2. `.soe/product.md` — product rules (deliverables, tiers, dependencies)
...
Check against rules defined in `.soe/product.md`:
...
- **PASS** → Return to `loop-execution-evaluator` → the orchestrator marks complete
- **FAIL** → Return to `loop-execution-evaluator` → the orchestrator dispatches `loop-fixer`
