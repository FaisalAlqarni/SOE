...
export async function completeTrack(dir, provenance, opts = {}) {
  ...
  // on disk. The orchestrator SHOULD always supply `provenance.tier` +
  // `provenance.reviews`; when `tier` is absent this check is skipped
  // (back-compat with pre-existing provenance records).
  if (provenance.tier) {
    const r = requireTrackLenses(provenance.tier, provenance.reviews || [], {
