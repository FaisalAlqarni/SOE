 ...
 *
 * ORDERING (load-bearing): validate a worker's return with `parse()` BEFORE
 * removing its worktree/scratch dir. The returned `path` must still exist on
 * disk at validation time, or the existence check below wrongly rejects an
 * honest worker. Orchestrators MUST call parse() first, then clean up the
 * worktree only after the envelope has been accepted.
 */
