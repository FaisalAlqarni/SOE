...
export function defaultConfig(orchestratorModel) {
  return {
    ...
    // Model tiers as latest FULL ids (not aliases — aliases lag, e.g. `sonnet`
    // still resolves to claude-sonnet-4-6). Normal tiering rides each agent's
    // own `model:` frontmatter; `lib/model-resolve.js` reads this block +
    // `fable_enabled` to resolve the ONE gated tier (strategist) at dispatch.
    models: {
      orchestrator: orchestratorModel, // the session model (documentation)
      reasoner: 'claude-opus-4-8',
      worker: 'claude-sonnet-5',
      cheap: 'claude-haiku-4-5',
      strategist: 'claude-fable-5',
    },
    // The Fable gate: set false to route the strategist tier to the reasoner
    // (Opus) instead of Fable 5 — e.g. to cap Fable spend. Availability (is the
    // user on a Fable plan at all) is handled separately by capability discovery.
    fable_enabled: true,
  };
}
