...
/**
 * True iff `path` alone matches any HIGH-RISK marker's `path` regex (auth,
 * authz, payment, crypto, secrets, migrations, PII, prod-config). This is a
 * path-only predicate for callers (e.g. lib/hitl.js's `isSensitivePath`) that
 * have just a file path, not full diff content — content-only markers
 * (deletion, force-push) have no `path` regex and are never evaluated here.
 *
 * @param {string} path
 * @returns {boolean}
 */
export function hasRiskyPathMarker(path) {
  const p = String(path);
  return MARKERS.some((m) => m.path && m.path.test(p));
}
